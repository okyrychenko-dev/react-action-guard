...
Automatically block UI based on dynamic conditions.
This is an advanced hook for cases where polling is an acceptable tradeoff.
...
Use this hook when the blocking condition does not already have a clean event-driven trigger.
If you already have explicit async state or a boolean flag, prefer `useAsyncAction` or `useBlocker`.
...
```typescript
interface ConditionalBlockerConfig<TState = unknown> extends Omit<BlockerConfig, "scope"> {
  scope: string | ReadonlyArray<string>; // Required scope(s) to block
  condition: (state?: TState) => boolean; // Condition to evaluate
  checkInterval?: number; // Check frequency in ms (default: 1000)
  state?: TState; // Optional state to pass to condition
}
```
...
This hook is best for edge cases like external quotas, derived limits, or state that is only available via periodic checks.
...
function ProtectedFeature() {
  ...
  useConditionalBlocker("access-control", {
    ...
    condition: () => {
      ...
      if (!permissions.includes("feature:write")) {
        return true;
      }
...
function DynamicValidation() {
  ...
  useConditionalBlocker("dynamic-validation", {
    ...
    condition: (state: ValidationState) => {
      if (state.required && value.length === 0) {
        return true;
      }
