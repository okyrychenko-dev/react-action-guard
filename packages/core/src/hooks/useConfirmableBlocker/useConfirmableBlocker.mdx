...
Create actions that require user confirmation before execution.
This is an advanced flow built on top of the core blocking primitives.
...
Use it when a workflow needs both confirmation UI and blocking during execution.
If you only need async blocking, start with `useAsyncAction`.
...
This hook is most useful for destructive or irreversible actions.
