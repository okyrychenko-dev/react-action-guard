...
import * as UseBlockerStories from "./useActionBlocker.stories";
...
| `isActive`  | `boolean`       | ❌       | `true`  | Controls whether the blocker is currently active.                          |
...
interface BlockerConfig {
  ...
  /** Auto-remove blocker after N milliseconds */
  timeout?: number;

  /** Optional callback when timeout triggers */
  onTimeout?: (blockerId: string) => void;
}
```

When `isActive` is `true`, updates to `config` are applied to the active blocker automatically.
