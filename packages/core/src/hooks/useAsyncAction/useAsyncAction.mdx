...
| ---------- | ----------------------- | -------- | ---------- | ----------------------------------------------------------------- |
| `actionId` | `string`                | ✅       | -          | Unique identifier for this action. Used in blocker ID generation. |
| `scope`    | `string \| string[]`    | ❌       | `'global'` | Scope(s) to block during execution.                               |
| `options`  | `UseAsyncActionOptions` | ❌       | -          | Optional timeout settings (`timeout`, `onTimeout`).               |
...
- ✅ **API calls** with automatic UI blocking
- ✅ **Form submissions** that take time
- ✅ **Data fetching** operations
- ✅ **File uploads/downloads**
- ✅ **Any async operation** that should block UI

Consider alternatives when you need:

- ❌ **User confirmation** → Use `useConfirmableBlocker`
- ❌ **Manual control** → Use `useBlocker`
- ❌ **Scheduled operations** → Use `useScheduledBlocker`
- ❌ **Condition-based blocking** → Use `useConditionalBlocker`
