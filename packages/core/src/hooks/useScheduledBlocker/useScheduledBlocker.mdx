...
Schedule UI blocking for specific time periods.
This is an advanced hook for maintenance windows, timed restrictions, or other schedule-driven flows.
...
Use it when blocking is driven by time rather than by a user-triggered async action.
If a user interaction starts the work, prefer `useAsyncAction` or `useBlocker`.
...
| `config`    | `ScheduledBlockerConfig` | ✅       | Configuration including schedule and callbacks |
...
interface BlockingSchedule {
  start: string | Date | number; // When to start blocking
  end?: string | Date | number; // When to end (optional if duration provided)
...
This hook is typically used for operational scenarios such as maintenance windows rather than day-to-day button handling.
...
- ✅ **Time-limited features** (early access periods, trials)
- ✅ **Scheduled deployments** (zero-downtime releases)
- ✅ **Timed restrictions** (office hours only, rate limiting)
- ✅ **Automated blocking** (time-based access control)
...
- ❌ **Dynamic conditions** → Use `useConditionalBlocker`
...
function MaintenanceBanner() {
  ...
  return (
    ...
    >
      {isActive ? (
        <>🔧 System Under Maintenance - Please Wait</>
      ) : (
        <>⚠️ Scheduled maintenance at {schedule.toLocaleTimeString()}</>
      )}
    </div>
  );
}
