...
- **Default global** checking - when omitted, checks `"global"` scope
...
| --------- | -------------------- | -------- | ----------- | ------------------------------------------------- |
| `scope`   | `string \| string[]` | ❌       | `"global"` | Scope(s) to check. If omitted, checks global scope. |
...
2. **Without scope parameter**: Any blocker affects global scope (including direct `"global"` blockers)
...
Check global blocking state by omitting `scope`:

```tsx
function AppStatusIndicator() {
  const isGloballyBlocked = useIsBlocked(); // Defaults to "global"

  return (
    <StatusBar>
      {isGloballyBlocked ? <span>⚠️ Global actions are currently blocked</span> : <span>✅ Global scope is clear</span>}
    </StatusBar>
  );
}
```
...
export function useIsBlocked(scope?: string | string[]): boolean {
